...
// import { getAllOrigins } from "@/services/originService"; // 👈 Update import
...
import { ImageUploader } from "./ImageUploader";
import { ProductPreview } from "./ProductPreview";
import { categoryTypes, measuringUnitTypes } from "../../lib/data";
import { saveProduct, updateProduct } from "@/services/productService";
import LoadingOverlay from "../LoadingOverlay";
import NotificationService from "@/utils/NotificationService";
import { v4 as uuidv4 } from "uuid";
...
export function ProductForm({ product, onSave, onCancel }: ProductFormProps) {
  ...
  const [originList, setOriginList] = useState<
    { originID: number; stateLocation: string; factoryName: string }[]
  >([]);
  const idempotencyKeyRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    const fetchOriginOptions = async () => {
      try {
        const response = await getAllOrigins(0, 100);
        console.log("Origin data:", response);
        if (Array.isArray(response.data.data?.origins)) {
          setOriginList(response.data.data.origins);
        } else {
          console.warn("Unexpected origin data format:", response);
        }
      } catch (error) {
          ...
          >
              ...
              >
                ...
                {originList.map((origin) => (
                  <option key={origin.originID} value={origin.originID}>
                    {origin.stateLocation} - {origin.factoryName}
                  </option>
                ))}
              </select>
              <p className="text-red-600 text-sm mt-1">
            ...
            <div>
              ...
              <ImageUploader
                value={formValues.productImageURLs || []}
                onChange={(urls) =>
                  form.setValue("productImageURLs", urls)
                }
              />
              <p className="text-red-600 text-sm mt-1">
                {form.formState.errors.productImageURLs?.message}
